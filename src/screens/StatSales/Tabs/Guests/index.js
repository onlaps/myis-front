...
const Comp = forwardRef((props, ref) => {
  ...
  const [cType, setCType] = useState("hours_sum");
  ...
  const getData = async () => {
    try {
      ...
      const { data } = await dispatch(
        call({ url: `statistic/sales/guests`, method: "POST", data: values })
      );
  ...
  const getChart = () => {
    ...
    if (cType === "guests") {
      series.push(addColumn("total"));
      series.push(addColumn("total_new"));
      series.push(addColumn("total_card"));
    } else series.push(addColumn(cType));
    ...
    return (
      <Card>
        <Radio.Group
          ...
          value={cType}
        >
          <Radio.Button value="hours_sum">Выручка</Radio.Button>
          <Radio.Button value="guests">Гости</Radio.Button>
          <Radio.Button value="avg">Средний чек</Radio.Button>
          <Radio.Button value="avg_hours">Среднее время</Radio.Button>
        </Radio.Group>
        <ReactApexChart
          options={options}
          series={series}
